...
export const CartazPreviewVagaInterna = ({ data }: CartazPreviewVagaInternaProps) => {
  ...
  const drawCartaz = async () => {
    ...
    const leftWidth = Math.round(baseWidth * 0.45); // 45% de 1080
    ...
    ctx.fillText('conosco', contentX, topOffset + contentOffset + 375);

    // Badge PCD ao lado do título
    if (data.isPcd) {
      const textWidth = ctx.measureText('conosco').width;
